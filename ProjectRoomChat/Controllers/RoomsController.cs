...
namespace ProjectRoomChat.Controllers
{
    ...
    public class RoomsController : ControllerBase
    {
        ...
        {
            var rooms = await _context.Rooms.Include(x => x.Admin).ToListAsync();
        ...
        {
            if (_context.Rooms.Any(x => x.Name == viewModel.Name))
            {
            ...
            var createRoom = _mapper.Map<Room, RoomViewModel>(room);
            await _hubContext.Clients.All.SendAsync("addChatRoom", createRoom);

            return CreatedAtAction(nameof(Get), new { id = room.Id }, createRoom);
        }
